...
def checkRecursive3(g,x,y,p,i,j,f=False):
    if not inside(x+j,y+i) or g[y+i][x+j] == ' ': return False,f
    if g[y+i][x+j] == p: return True,f

    if checkRecursive3(g,x+j,y+i,p,i,j):
        f = True
        g[y][x] = p
        g[y+i][x+j] = p
        #checkCoords(g,x,y,p)
...
def checkCoords(g,x,y,p):
    if g[y][x] == ' ':
        for i in range(-1, 2):
            for j in range(-1, 2):
                if i == 0 and j == 0: continue
                inutile, flag = checkRecursive3(g,x,y,p,i,j)
...
while True:
    ...
    checkCoords(matrix, coords[0], coords[1], player)
